...
public class HttpProxyInterceptPipeline implements Iterable<HttpProxyIntercept> {
  ...
  public void afterResponse(Channel clientChannel, Channel proxyChannel,  HttpRequest httpRequest,HttpResponse httpResponse)
      throws Exception {
    if (this.pos3 == 0) {
      this.httpResponse = httpResponse;
    }
    if (this.pos3 < intercepts.size()) {
      ...
      intercept.afterResponse(clientChannel, proxyChannel, httpRequest,this.httpResponse, this);
    }
    this.pos3 = 0;
  }

  public void afterResponse(Channel clientChannel, Channel proxyChannel,  HttpRequest httpRequest, HttpContent httpContent)
      throws Exception {
    if (this.pos4 < intercepts.size()) {
      HttpProxyIntercept intercept = intercepts.get(this.pos4++);
      intercept.afterResponse(clientChannel, proxyChannel, httpRequest,httpContent, this);
    }
    this.pos4 = 0;
  }
